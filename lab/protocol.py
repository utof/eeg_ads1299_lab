...
import struct
import numpy as np
from .adc import MIN_CODE, MAX_CODE, GAINS, RATES
...
def native_frame(codes, status=0xC00000):
    values=list(codes)
...
def parse_native(frame,channels):
    ...
    return (int.from_bytes(frame[:3],'big'),
...
class Packet:
    ...
    def encode(self):
        ...
        header=HEADER.pack(MAGIC,VERSION,len(self.codes),self.flags,self.gain,
                           self.fs_hz,self.sequence,self.time_us,self.overruns)
    ...
    def decode(data):
        ...
        magic,ver,n,flags,gain,fs,seq,t,over=HEADER.unpack_from(data)
    ...
    def feed(self,chunk):
        ...
        while len(self.buffer)>=HEADER.size:
            ...
            if ver!=VERSION or n not in CHANNELS:
                del self.buffer[0]; self.discarded_bytes+=1; self.invalid_candidates+=1; continue
...
class Tracker:
    ...
    def observe(self,p):
        ...
        if self.last is not None:
            ...
            if ds>=1<<31 or dt>=1<<31:
                self.stale+=1; return None
            gap=ds-1
            self.missing+=gap
            expected=ds*1e6/p.fs_hz
            if abs(dt-expected)>max(500,expected*.1):
    ...
    def summary(self):
        return {k:getattr(self,k) for k in ('accepted','missing','duplicates','stale','timing_anomalies')}
