...
Neither Python types nor frozen values can prevent all disk errors, external file edits, process termination, reflection, or concurrent writers. At the PR #13 snapshot, CSVs/netlists/logs were not generation-bound and arbitrary malformed CLI rejection was outside its guarantee. The later issue #15 / PR #19 contract adds per-run manifests, a last-success pointer, mandatory reader identity, digest checks and explicit writer exclusion. See `docs/REV_A_RUN_IDENTITY.md`. That later filesystem work still does not establish authenticity, power-loss durability or model realism.
