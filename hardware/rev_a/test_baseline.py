...
from check_baseline import HERE, load_documents, totals, validate
...
class BaselineTests(unittest.TestCase):
    ...
    def test_markdown_bom_matches_json(self):
        ...
        for row in self.bom['line_items']:
            ...
            expected = f"`{row['mpn']}` | {row['quantity']} | {row['population'].upper()} | ${value:.2f} |"
            self.assertIn(expected, doc)
        self.assertIn(f"Planning subtotal: ${totals(self.bom)['planning_total']:.2f}", doc)

    def test_costs_and_optional_exclusion(self):
    ...
    def test_every_gate_is_fail_closed(self):
    ...
    def test_header_pin_missing(self):
        del self.profile['interface_headers']['J_DIG']['pin_map']['20']
        self.assert_rejected('pins 1 through 20')

    def test_cli_works_from_other_working_directory(self):
        with tempfile.TemporaryDirectory() as directory:
            proc = subprocess.run([sys.executable, str(HERE/'check_baseline.py')], cwd=directory,
                                  text=True, capture_output=True, check=False)
        self.assertEqual(proc.returncode, 0, proc.stderr)
        self.assertIn('NOT hardware or safety validation', proc.stdout)

    def test_cli_bad_json_fails_cleanly(self):
        with tempfile.TemporaryDirectory() as directory:
            Path(directory, 'board_profile.json').write_text('{broken', encoding='utf-8')
            proc = subprocess.run([sys.executable, str(HERE/'check_baseline.py'), '--directory', directory],
                                  text=True, capture_output=True, check=False)
