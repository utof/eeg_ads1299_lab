...
from lab.protocol import FLAG_SYNTHETIC, Packet
from lab.signals import SyntheticConfig, generate


@pytest.mark.parametrize("channels,line,fs", [(6, 60, 250), (8, 50, 500)])
def test_other_variants_end_to_end(channels: int, line: int, fs: int) -> None:
    data = generate(
        SyntheticConfig(channels=channels, line_hz=line, fs_hz=fs, sessions=3, blocks_per_session=4)
    )
    report, _ = analyze(data, permutations=0)
    assert report["balanced_accuracy"] > 0.8
    assert len(report["feature_names"]) == 3 * channels


def test_null_effect_is_not_a_perfect_decoder() -> None:
    ...
    assert 0.7 < report["closed_open_alpha_ratio"] < 1.3
    assert 0.25 < report["balanced_accuracy"] < 0.75


def make_capture(tmp_path: Path, drop_every: int = 0) -> Path:
    cfg = ADCConfig()
    t = np.arange(8 * cfg.fs_hz) / cfg.fs_hz
    codes, _ = quantize(10e-6 * np.sin(2 * np.pi * 10 * t), cfg)
    raw = tmp_path / "test.bin"
    with raw.open("wb") as f:
        for i, code in enumerate(np.atleast_1d(codes)):
...
def test_capture_inspection_preserves_units(tmp_path: Path) -> None:
    report = inspect_capture(make_capture(tmp_path), tmp_path / "inspect")
    assert report["four_second_spectral_windows"] == 2
    assert report["peak_0p5_40_Hz_by_channel"] == [10.0] * 4
    assert report["rms_0p5_40_Hz_uV_by_channel"][0] == pytest.approx(10 / (2**0.5), rel=0.002)
    assert report["rail_code_samples_by_channel"] == [0] * 4


def test_capture_inspection_does_not_bridge_gaps(tmp_path: Path) -> None:
...
def test_capture_inspection_requires_metadata(tmp_path: Path) -> None:
    p = tmp_path / "missing.csv"
    p.write_text("")
    with pytest.raises(ValueError, match="metadata JSON"):
        inspect_capture(p, tmp_path / "inspect")


def test_firmware_is_bench_gated_and_halt_powers_down() -> None:
...
def test_cli_rejects_negative_permutation_count(tmp_path: Path) -> None:
    root = Path(__file__).resolve().parents[1]
