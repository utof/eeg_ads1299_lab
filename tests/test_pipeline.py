...
from lab.dsp import psd,bandpower,quality_flags,analyze
from lab.pipeline import save_data,load_data


def test_known_sine_power_and_peak():
    fs=250;t=np.arange(2500)/fs;x=10e-6*np.sin(2*np.pi*10*t)
    assert bandpower(x,fs,8,13)==pytest.approx(50e-12,rel=.002)
    f,p=psd(x,fs);assert f[np.argmax(p)]==10


def test_fixed_artifact_rule():
...
def test_excessive_offset_cannot_be_repaired_by_filter():
    data=generate(SyntheticConfig(sessions=3,blocks_per_session=4,differential_offset_mV=300))
    assert data['invalid'].mean()>.9
    with pytest.raises(ValueError,match='All epochs rejected'):analyze(data,permutations=0)
