...
from lab.protocol import Packet, StreamDecoder, Tracker, native_frame, parse_native, signed24


@pytest.mark.parametrize("n", [4, 6, 8])
def test_native_and_packet_roundtrip(n: int) -> None:
...
def test_sign_extension(value: int) -> None:
...
def test_corrupted_truncated_and_wrong_native_status_rejected() -> None:
    raw = bytearray(Packet(1, 4000, (1, 2, 3, 4)).encode())
    for i in range(len(raw)):
        broken = raw.copy()
        broken[i] ^= 1
        with pytest.raises(ValueError):
            Packet.decode(broken)
    with pytest.raises(ValueError):
        Packet.decode(raw[:-1])
    with pytest.raises(ValueError):
        parse_native(bytes(15), 4)


def test_stream_chunking_noise_and_resynchronization() -> None:
    packets = [Packet(i, i * 4000, (i, -i, 0, 3)) for i in range(20)]
    bad = bytearray(packets[3].encode())
    bad[-1] ^= 3
    stream = (
        b"boot banner\n"
        + b"".join(p.encode() for p in packets[:10])
        + bytes(bad)
        + b"noise"
        + b"".join(p.encode() for p in packets[10:])
    )
    ...
    while offset < len(stream):
        ...
        out += decoder.feed(stream[offset : offset + n])
        offset += n
    assert out == packets
    assert decoder.discarded_bytes > 0 and decoder.invalid_candidates > 0


def test_gap_duplicate_out_of_order_and_counter_wrap() -> None:
    tr = Tracker()
    seq = [0xFFFFFFFE, 0xFFFFFFFF, 0, 2, 2, 1, 3]
    times = [0xFFFFE0C0, 0xFFFFF060, 0, 8000, 8000, 4000, 12000]
    for s, t in zip(seq, times, strict=True):
        tr.observe(Packet(s, t, (0, 0, 0, 0)))
    assert tr.accepted == 5 and tr.missing == 1 and tr.duplicates == 1 and tr.stale == 1
    assert tr.elapsed_us == 20000


def test_configuration_changes_require_new_capture() -> None:
    tr = Tracker()
    tr.observe(Packet(0, 0, (0, 0, 0, 0)))
    with pytest.raises(ValueError):
        tr.observe(Packet(1, 4000, (0, 0, 0, 0), gain=12))
