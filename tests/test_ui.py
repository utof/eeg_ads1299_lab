...
@pytest.mark.native
def test_interactive_headroom_calculator_matches_expected_boundaries() -> None:
    node = shutil.which("node")
    if not node:
        pytest.skip("Node unavailable; UI arithmetic not rerun")
    script = """
...
def test_html_has_no_remote_dependencies_and_keeps_wiring_boundary() -> None:
