...
@pytest.mark.native
def test_native_cpp_encoder_matches_python_byte_for_byte(tmp_path: Path) -> None:
    compiler = shutil.which("g++") or shutil.which("clang++")
    if not compiler:
        pytest.skip("Native C++ compiler unavailable; inspect validation scope")
    ...
    built = subprocess.run(
        [
            ...
            "-Werror",
            "-pedantic",
            str(ROOT / "tests/native_core_test.cpp"),
            "-o",
            str(exe),
        ],
        capture_output=True,
...
def test_real_local_udp_socket_packet_exchange() -> None:
    with (
...
def test_capture_decoder_preserves_missing_samples(tmp_path: Path) -> None:
...
def test_cli_help_works_from_another_directory(tmp_path: Path) -> None:
    proc = subprocess.run(
        [sys.executable, str(ROOT / "run_lab.py"), "--help"],
        cwd=tmp_path,
        capture_output=True,
        text=True,
    )
