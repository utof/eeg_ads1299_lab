...
from lab.analog import InputNetwork, export_spice, transfer


def test_matches_closed_form_symmetric_resistor_capacitor_network() -> None:
    p = InputNetwork(c_electrode_p=0, c_electrode_n=0)
    f = np.geomspace(0.1, 100000, 100)
    r = p.r_electrode_p + p.r_series_p
    analytic = 1 / (
        1 + r / p.r_input_p + 2j * np.pi * f * r * (p.c_common_p + 2 * p.c_differential)
    )
...
def test_perfect_symmetry_cancels_common_mode() -> None:
    h = transfer(np.geomspace(0.1, 1e5, 200), InputNetwork(), "common")
    assert max(abs(h)) < 1e-12


def test_electrode_and_capacitor_mismatch_create_differential_error() -> None:
    p = InputNetwork()
...
def test_electrode_sweep_remains_finite(r: int) -> None:
    h = transfer([0.1, 10, 40, 50, 100000], replace(InputNetwork(), r_electrode_n=r))
    assert np.isfinite(h).all() and np.max(abs(h)) <= 1.001


def test_netlist_has_same_values_and_no_false_hardware_claim(tmp_path: Path) -> None:
