...
def main(out: Path | None = None) -> int:
    ...
    try:
        if proc.stdout is None:
            raise RuntimeError("Receiver output pipe is unavailable")
        ...
        output: tuple[str, str] = proc.communicate(timeout=15)
        stdout, stderr = output
        (out / "receiver.log").write_text(line + stdout + "\n" + stderr)
        if proc.returncode:
            raise RuntimeError("Receiver failed; inspect receiver.log")
    finally:
    ...
    report = {
        "scope": "real localhost UDP; synthetic payload; NO hardware test",
        "deliberately_omitted_sequences": [1000, 2000],
        "sent": sent,
        "decoded": decoded,
        "quality": quality,
    }
        ...
        json.dumps(
            {
                ...
                "peak_hz": quality["peak_0p5_40_Hz_by_channel"],
            },
            indent=2,
        )
    )
