...
def main(argv=None):
    p=argparse.ArgumentParser(description='ADS1299 learning lab: models, tests, and bench-only transport')
    ...
    circuits=sub.add_parser('circuits',help='Solve a passive model; export SPICE; compare when installed')
    circuits.add_argument('--out',type=Path,default=ROOT/'results/circuits')
    circuits.add_argument('--require-ngspice',action='store_true')
    verify=sub.add_parser('verify',help='Run automated tests and record exact verification scope')
    ...
    replay=sub.add_parser('replay',help='Send synthetic UDP packets at the nominal acquisition rate')
    replay.add_argument('--host',default='127.0.0.1');replay.add_argument('--port',type=int,default=9000)
    replay.add_argument('--seconds',type=float,default=3);replay.add_argument('--channels',type=int,choices=[4,6,8],default=4)
    replay.add_argument('--drop-every',type=int,default=0)
    decode=sub.add_parser('decode',help='Decode transport file to CSV without interpolating gaps')
    decode.add_argument('input',type=Path);decode.add_argument('--out',type=Path,default=ROOT/'results/capture.csv')
    decode.add_argument('--vref',type=float,default=4.5)
    inspect=sub.add_parser('inspect',help='Gap-safe quality and PSD report from decoded bench CSV')
    inspect.add_argument('input',type=Path)
    inspect.add_argument('--out',type=Path,default=ROOT/'results/capture_quality')
    serial=sub.add_parser('serial',help='USB bench capture ONLY, all body electrodes disconnected')
    serial.add_argument('--port',required=True);serial.add_argument('--out',type=Path,default=ROOT/'results/serial.bin')
    ...
    try:
        if args.command=='demo':
            ...
            cfg=SyntheticConfig(**json.loads(args.config.read_text())) if args.config else SyntheticConfig()
            changes={k:getattr(args,k) for k in ['seed','channels','line_hz'] if getattr(args,k) is not None}
            if args.null_effect:changes['null_effect']=True
            cfg=replace(cfg,**changes)
            if args.permutations<0:raise ValueError('permutations must be >=0')
            report=run_demo(args.out,cfg,args.permutations,not args.no_plots)
            print(json.dumps({k:report[k] for k in ['kind','accepted_epochs','rejected_epochs',
                            'closed_open_alpha_ratio','balanced_accuracy','baseline_balanced_accuracy','block_permutation_mean']},indent=2))
            ...
            print(json.dumps(replay_packets(args.host,args.port,args.seconds,args.channels,drop_every=args.drop_every),indent=2))
